...
function buildCodeSearchInterface() {
  const codeSearchContainer = document.createElement("div") as HTMLDivElement;
...
function buildPlayerContents(playerContainer: HTMLDivElement) {
  ...
  const toggleInteraction: HTMLButtonElement = components.buildPlayerBtn("Toggle Interaction");
  ...
  colorPicker.addEventListener("input", listeners.setColor);
